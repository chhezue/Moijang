...
export class VerificationService {
  ...
  async sendCode(dto: RequestSendCodeDto): Promise<ResponseSendCodeDto> {
    ...
    if (exist) {
      throw new BadRequestException('이미 해당 이메일로 가입한 사용자가 존재합니다.');
    }
    ...
    try {
      ...
      throw new InternalServerErrorException('인증 코드 메일 발송에 실패했습니다.');
    }
  ...
  // 웹메일로 받은 코드 인증
  async confirmCode(dto: RequestConfirmCodeDto): Promise<ResponseConfirmCodeDto> {
    const verification = await this.verificationRepository.getSession(dto.verificationId);

    // TTL이 만료되어 토큰이 삭제된 경우
    if (!verification || Object.keys(verification).length === 0) {
      throw new NotFoundException('인증 요청이 만료되었거나 존재하지 않습니다.');
    }
