...
export class ParticipantService {
  ...
  async updateParticipant(
    ...
    if (gb.groupBuyingStatus !== GroupBuyingStatus.RECRUITING) {
      throw new BadRequestException('참여자 정보 수정은 모집 중 상태일 때만 가능합니다.');
    }
    ...
    const beforeTotalCount = await this.participantRepository.getTotalCount(gbId);
    ...
    const updatedParticipant = await this.participantRepository.findOneAndUpdate(
      { gbId, userId },
      updateDto,
      { new: true },
    );
  ...
  async confirmPayment(gbId: string, userId: string): Promise<Participant> {
    ...
    const groupBuying: any = await this.groupBuyingRepository.findOneByGbId(gbId);
    if (groupBuying.groupBuyingStatus !== GroupBuyingStatus.PAYMENT_IN_PROGRESS) {
      throw new BadRequestException('참여자 입금 확정은 입금 진행 중 상태일 때만 가능합니다.');
    }
    ...
    const allConfirmPayment = await this.participantRepository.checkAllUserPayment(gbId);

    if (allConfirmPayment) {
      await this.groupBuyingRepository.updateStatus(gbId, GroupBuyingStatus.ORDER_PENDING);
